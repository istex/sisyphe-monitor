...
MonitorController.prototype.router = function (workers) {
  ...
  for (var i = 0; i < workers.length; i++) {
    ...
    if (worker.name === 'walker-fs' || worker.name === 'start' || worker.name === 'end') continue;
    nbWorkers++;
    ...
    } else if (worker.waiting || worker.maxFile < this.maxFile) {
    ...
    if (+this.maxFile < +worker.maxFile - 1) {
      // -1 is for the init task
